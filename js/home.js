...
function handleRoute() {
  ...
  else if (hash === 'regulation') showView('regulation');
  else showView('home');
}
...
window.addEventListener('load', () => {
  ...
  // Back button in regulation view
  document.addEventListener('click', (e) => {
    if (e.target && e.target.id === 'btnBackHomeReg') location.hash = '#/';
  });
...
function showView(name) {
  ...
  if (name === 'regulation') renderRegulation();
}
...
// renderers
function renderAll() { renderHomeTop5(); renderPublicRanking(); renderRankingPreview(); updateNextSaturdayTag(); if (!document.getElementById('view-regulation').classList.contains('hidden')) renderRegulation(); }

async function renderRegulation() {
